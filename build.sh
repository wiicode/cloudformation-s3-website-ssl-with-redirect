...
# Loop through subdomains and create stacks
while read -r SUBDOMAIN; do
  STACK_NAME=$(echo "$SUBDOMAIN" | cut -d. -f1)
  echo "Creating stack for subdomain: $SUBDOMAIN (stack name: $STACK_NAME)"
  ...
  if [[ $? -eq 0 ]]; then
    echo "Stack creation initiated for $SUBDOMAIN"
  else
    echo "Failed to initiate stack creation for $SUBDOMAIN"
  fi
done < "$SUBDOMAIN_FILE"
